<script lang="ts">
    ...
    $: filteredBookings = bookings.filter((booking) => {
        return booking.startTs.day == date.day && booking.startTs.month == date.month && booking.startTs.year == date.year;
    });
    ...
    const compareTs = (a: Timestep|null, b: Timestep|null): number => {
        ...
        if(a.hStart == b.hStart && a.mStart == b.mStart) 
            return 0;
     ...
     * @param end
     */
    const correctOrder = (start: Timestep|null, end: Timestep|null): [typeof start|typeof end, typeof end|typeof start] => {
     ...
     */
    const checkInBounds = (start: Timestep|null, end: Timestep|null, timestep: Timestep): boolean => {
        [start, end] = correctOrder(start, end);
        
        if(start == null || end == null) return false;

        if(compareTs(start, timestep) < 0)
            return false;

        if(compareTs(timestep, end) < 0)
            return false;
...
<div class="booking-day">
    <div class="bg" on:mouseleave={() => {
        ...
        {#each timesteps as timestep, timestepIdx}
            ...
            <div 
                class="timestep" class:top={timestep.type == 'top'} 
                class:selected={isSelected} class:selectStart={startSlot == timestep} class:selectEnd={timestepIdx > 0 && endSlot == timesteps[timestepIdx-1]}
                class:selectEndDirect={endSlot == timesteps[timestepIdx]} 
                role="cell" on:mousedown={(ev) => {
    ...
    <div class="fg">
        {#each filteredBookings as booking}
            ...
            style="height: {booking.endTs.diff(booking.startTs).as('hours') * bookingHourLengthPx}px; top: { bookingTopPadPx + ((booking.startTs.hour + (booking.startTs.minute / 60)) * bookingHourLengthPx) }px; left: {bookingLeftPadPx}px; width: calc(100% - {bookingLeftPadPx}px);"
            on:click={(ev) => {
                emitter("edit", booking);
                ev.preventDefault();
            }}>
                <span class="project">{booking.project}</span>
                <span class="task">{booking.task}</span>
            </button>
        {/each}
    </div>
</div>
